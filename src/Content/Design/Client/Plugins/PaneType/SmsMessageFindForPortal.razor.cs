...
public partial class SmsMessageFindForPortal : IPaneDisplay, IDisposable
{
    ...
    {
        var portalId = Path!.Id("portal") ?? Id;

        Model = new(EventBus, ScrollService, SmsMessageService, portalId);
    ...
    IHandle<SmsMessageAdded>, IHandle<SmsMessageSaved>, IHandle<SmsMessageRemoved>
{
    ...
    public SmsMessageFindForPortalModel(IEventBus eventBus, IScrollService scrollService, ISmsMessageService smsMessageService, Guid? portalId)
        : base(scrollService)
    {
    ...
    public async Task Reset()
    {
        ...
        Search.Direction = SmsMessage.Directions.Inbound;

...
public partial class SmsEdit : IPaneDisplay, IDisposable
{
    ...
    protected override async Task OnInitializedAsync()
    {
        ...
        Model = new(Path, Id, IsNew, membershipId, portalId, EventBus, Navigator, SmsService, MembershipService, TokenService);
    ...
    IHandle<SmsTemplateAdded>, IHandle<SmsTemplateSaved>, IHandle<SmsTemplateRemoved>
{
    ...
    {
        var response = await WithAlerts(() => _smsService.FetchSmsTemplates(new()), false);
        if (response.Ok) SmsTemplates = response.Value.ToObservable();
    }
    ...
    private void SetSms(Sms sms)
    {
        ...
        _navigator.UpdateTitle(_path, Entity.Body);
    }
}
...
public partial class PageEditForPane : IPaneDisplay, IDisposable
{
    ...
    public PageEditForPaneModel Model { get; set; } = null!;

    protected override async Task OnInitializedAsync()
    {
        Model = new(Path, Id, IsNew, EventBus, Navigator, ScrollService, PanePageService);
...
public class PageEditForPaneModel(
    String? path,
    Guid? id,
    Boolean isNew,
    IEventBus eventBus,
    ...
    : PageEditModelBase(path, id, isNew, eventBus, navigator, scrollService)
{
    protected override async Task<Response<IList<Orderable>>> FetchStatuses() =>
        await panePageService.FetchContentStatusNames(new());

    protected override async Task<Response<Page?>> FetchPage(Guid? id) =>
        await panePageService.FetchPage(new(new()
        {
            Page = new() { Id = id },
        }));

    protected override async Task<Response<Page?>> AddPage(Page page) =>
        await panePageService.AddPage(new(new() { Page = page }));

    protected override async Task<Response> SavePage(Page page) =>
        await panePageService.SavePage(new(new() { Page = page }));
}
    ...
    : IEmailService
{
    ...
    public async Task<Response<IList<EmailSent>>> SearchSentForEmail(Request<EmailSentSearch> request)
    {
        return await wrappers.Try<IList<EmailSent>>(request, async response =>
        {
            var emails = await EmailSentSelectWhereForEmail.Execute(Connection, request.SessionId, request.Value);
    ...
    public async Task<Response<IList<Named>>> FetchMembershipNamesForActivation(Request<Activation> request)
    {
        return await wrappers.Try<IList<Named>>(request, async response =>
            await EmailMembershipSelectForActivation.Execute(Connection, request.SessionId, request.Value.Id));
    }

    public async Task<Response<IList<Named>>> FetchStageNamesForActivation(Request<Activation> request)
    {
        return await wrappers.Try<IList<Named>>(request, async response =>
            await EmailStageSelectForActivation.Execute(Connection, request.SessionId, request.Value.Id));
    }

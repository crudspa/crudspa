...
public static class SmsMessageSelectWhereForMembership
{
    public static async Task<IList<SmsMessage>> Execute(String connection, Guid? sessionId, SmsMessageSearch search)
    {
        await using var command = new SqlCommand();
        command.CommandText = "ContentMessaging.SmsMessageSelectWhereForMembership";
    ...
    private static SmsMessage ReadSmsMessage(SqlDataReader reader)
    {
        return new()
        {
            ...
            ProviderMessageId = reader.ReadString(10),
        };
    }
}
...
public partial class EmailEdit : IPaneDisplay, IDisposable
{
    ...
    protected override async Task OnInitializedAsync()
    {
        ...
        var activationId = Path.Id("activation");
        var portalId = Path.Id("portal") ?? SessionState.Session.PortalId;

        Model = new(Path, Id, IsNew, membershipId, activationId, portalId, EventBus, Navigator, ScrollService, SessionState, EmailService, MembershipService);
    ...
    IHandle<EmailTemplateAdded>, IHandle<EmailTemplateSaved>, IHandle<EmailTemplateRemoved>
{
    ...
    public EmailEditModel(String? path, Guid? id, Boolean isNew, Guid? membershipId, Guid? activationId, Guid? portalId,
        IEventBus eventBus,
    ...
    public ObservableCollection<Named> Memberships
    {
        get;
        set => SetProperty(ref field, value);
    } = [];

    public ObservableCollection<Named> StageNames
    {
        get;
        set => SetProperty(ref field, value);
    } = [];
    ...
    public Boolean ShowStage => _activationId.HasValue;
    ...
    {
        await WithMany("Loading activation...", FetchMemberships(), FetchStageNames());
        await Refresh();
    ...
    public async Task Refresh()
    {
        if (IsNew)
        {
            ...
            SetEmail(new()
            {
                ...
                StageId = StageNames.FirstOrDefault()?.Id,
    ...
    {
        if (_membershipId is not null)
            return;

        if (_activationId.HasValue)
        {
            var activationResponse = await WithAlerts(() => _emailService.FetchMembershipNamesForActivation(
                new(new() { Id = _activationId })), false);
            if (activationResponse.Ok)
                Memberships = activationResponse.Value.ToObservable();
            return;
        }
        ...
        if (response.Ok)
            Memberships = response.Value.Select(x => new Named { Id = x.Id, Name = x.Name }).ToObservable();
    }

    public async Task FetchStageNames()
    {
        if (!_activationId.HasValue)
            return;

        var response = await WithAlerts(() => _emailService.FetchStageNamesForActivation(
            new(new() { Id = _activationId })), false);
        if (response.Ok)
            StageNames = response.Value.ToObservable();
    }

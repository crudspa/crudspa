...
public partial class SmsMessageFindForContactPhone : IPaneDisplay, IDisposable
{
    ...
    public SmsMessageFindForContactPhoneModel Model { get; set; } = null!;
    ...
    public void AddNew()
    {
        Navigator.GoTo($"{Path}/sms-message-{Guid.NewGuid():D}?state=new");
    }
}

public class SmsMessageFindForContactPhoneModel : FindModel<SmsMessageSearch, SmsMessage>,
    IHandle<SmsMessageAdded>, IHandle<SmsMessageSaved>, IHandle<SmsMessageRemoved>
{
    ...
    private readonly Guid? _contactPhoneId;
    private ObservableCollection<String> _sorts;
    private Boolean _resetting;

    public SmsMessageFindForContactPhoneModel(IEventBus eventBus, IScrollService scrollService, ISmsMessageService smsMessageService, Guid? contactPhoneId)
        : base(scrollService)
    {
    ...
    public async Task Reset()
    {
        ...
        Search.ParentId = _contactPhoneId;
    ...
    public override async Task Refresh(Boolean resetAlerts = true)
    {
        ...
        var response = await WithWaiting("Searching...", () => _smsMessageService.SearchForContactPhone(request), resetAlerts);

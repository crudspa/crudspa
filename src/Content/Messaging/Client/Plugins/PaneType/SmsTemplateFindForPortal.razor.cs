...
public partial class SmsTemplateFindForPortal : IPaneDisplay, IDisposable
{
    ...
    public SmsTemplateFindForPortalModel Model { get; set; } = null!;

    protected override async Task OnInitializedAsync()
    {
        var portalId = Path!.Id("portal") ?? Id ?? SessionState.Session.PortalId;

        Model = new(EventBus, ScrollService, SmsTemplateService, portalId);
...
public class SmsTemplateFindForPortalModel : FindModel<SmsTemplateSearch, SmsTemplate>,
    IHandle<SmsTemplateAdded>, IHandle<SmsTemplateSaved>, IHandle<SmsTemplateRemoved>
{
    ...
    public SmsTemplateFindForPortalModel(IEventBus eventBus, IScrollService scrollService, ISmsTemplateService smsTemplateService, Guid? portalId)
        : base(scrollService)
    {
    ...
    public override async Task Refresh(Boolean resetAlerts = true)
    {
        ...
        var response = await WithWaiting("Searching...", () => _smsTemplateService.SearchForPortal(request), resetAlerts);

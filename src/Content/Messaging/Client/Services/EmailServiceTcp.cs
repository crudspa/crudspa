...
public class EmailServiceTcp(IProxyWrappers proxyWrappers) : IEmailService
{
    ...
    public async Task<Response<IList<EmailSent>>> SearchSentForEmail(Request<EmailSentSearch> request) =>
        await proxyWrappers.Send<IList<EmailSent>>("EmailSearchSentForEmail", request);
    ...
    public async Task<Response<IList<Named>>> FetchMembershipNamesForActivation(Request<Activation> request) =>
        await proxyWrappers.Send<IList<Named>>("EmailFetchMembershipNamesForActivation", request);

    public async Task<Response<IList<Named>>> FetchStageNamesForActivation(Request<Activation> request) =>
        await proxyWrappers.Send<IList<Named>>("EmailFetchStageNamesForActivation", request);

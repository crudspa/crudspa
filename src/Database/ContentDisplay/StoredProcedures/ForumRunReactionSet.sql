create proc [ContentDisplay].[ForumRunReactionSet] (
...
begin
    ...
    if @sessionContactId is null or not exists (
        ...
        where comment.Id = @CommentId and comment.Removed = 0
            and (
    ...
    ) throw 51000, 'Comment was not found or is not accessible.', 1;

    begin transaction;

    declare @reactionId uniqueidentifier;
    declare @commentReactionId uniqueidentifier;

    select top 1 @reactionId = reaction.Id, @commentReactionId = commentReaction.Id
    from [Content].[CommentReaction] commentReaction with (updlock, serializable)
    inner join [Content].[Reaction] reaction with (updlock, serializable) on reaction.Id = commentReaction.ReactionId
    where commentReaction.VersionOf = commentReaction.Id and commentReaction.IsDeleted = 0
        and reaction.VersionOf = reaction.Id and reaction.IsDeleted = 0
        and commentReaction.CommentId = @CommentId and reaction.ById = @sessionContactId
    order by reaction.Reacted desc;

    declare @now datetimeoffset(7) = sysdatetimeoffset();

    if @Emoji is null
    begin
        if @commentReactionId is not null
        begin
            update [Content].[CommentReaction]
    ...
    else if @reactionId is not null
        update [Content].[Reaction]
        set Updated = @now, UpdatedBy = @SessionId, Reacted = @now, Emoji = @Emoji
        where Id = @reactionId;
    else
    begin
        set @reactionId = newid();
        set @commentReactionId = newid();

        insert [Content].[Reaction] (Id, VersionOf, Updated, UpdatedBy, ById, Reacted, Emoji)
        values (@reactionId, @reactionId, @now, @SessionId, @sessionContactId, @now, @Emoji);

        insert [Content].[CommentReaction] (Id, VersionOf, Updated, UpdatedBy, CommentId, ReactionId)
        values (@commentReactionId, @commentReactionId, @now, @SessionId, @CommentId, @reactionId);
    end

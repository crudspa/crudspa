create proc [ContentDisplay].[ForumRunCommentMediaUpdate] (
...
begin
    ...
    if not exists (
        ...
        where media.Id = @Id and media.CommentId = @CommentId and comment.Removed = 0
            and (
    ...
    ) throw 51000, 'Comment media cannot be edited.', 1;

    declare @now datetimeoffset(7) = sysdatetimeoffset();
    update [Content].[CommentMedia]
    set Updated = @now, UpdatedBy = @SessionId, [Type] = @Type,
        AudioId = @AudioId, ImageId = @ImageId, PdfId = @PdfId, VideoId = @VideoId, Ordinal = @Ordinal
    where Id = @Id;
end
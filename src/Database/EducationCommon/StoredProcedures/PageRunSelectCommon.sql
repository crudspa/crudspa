create proc [EducationCommon].[PageRunSelectCommon] (
     @Id uniqueidentifier
    ,@SessionId uniqueidentifier
) as

-- Keep top portion in sync with PageRunSelectContent
...
select
     activity.*
    ,activityElement.Id as Id
    ,activityElement.ElementId as ElementId
    ,activityElement.ActivityId as ActivityId
from [Education].[ActivityElement-Active] activityElement
    inner join [Education].[Activity-Deep] activity on activityElement.ActivityId = activity.Id
    inner join [Content].[Element-Active] element on activityElement.ElementId = element.Id
    inner join [Content].[Section-Active] section on element.SectionId = section.Id
where section.PageId = @Id
...
from [Education].[ActivityChoice-Deep] activityChoice
    ...
    inner join [Education].[ActivityElement-Active] activityElement on activityElement.ActivityId = activity.Id
    inner join [Content].[Element-Active] element on activityElement.ElementId = element.Id
    inner join [Content].[Section-Active] section on element.SectionId = section.Id
where section.PageId = @Id
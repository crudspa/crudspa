create proc [ContentMessaging].[EmailSelectWhereForPortal] (
...
as (
    select
        row_number() over (
            order by
                case when (@SortField = 'Send' and @SortAscending = 1) then email.Send end asc,
                case when (@SortField = 'Send' and @SortAscending = 0) then email.Send end desc,
                case when (@SortField = 'Subject' and @SortAscending = 1) then email.Subject end asc,
                case when (@SortField = 'Subject' and @SortAscending = 0) then email.Subject end desc,
                case when (@SortAscending = 1) then email.Id end asc,
                case when (@SortAscending = 0) then email.Id end desc
        ) as RowNumber

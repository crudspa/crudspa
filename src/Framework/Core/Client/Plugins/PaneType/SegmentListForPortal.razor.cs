...
public partial class SegmentListForPortal : IPaneDisplay, IDisposable
{
    ...
    public SegmentListForPortalModel Model { get; set; } = null!;
...
public class SegmentListForPortalModel : ListOrderablesModel<SegmentModel>,
    ...
    IHandle<LicenseAdded>, IHandle<LicenseSaved>, IHandle<LicenseRemoved>
{
    ...
    public SegmentListForPortalModel(IEventBus eventBus, IScrollService scrollService, ISegmentService segmentService, Guid? portalId)
        : base(scrollService)
    {
    ...
    public async Task Handle(SegmentAdded payload) => await Replace(payload.Id, payload.PortalId);

    public async Task Handle(SegmentSaved payload) => await Refresh(false);

    public async Task Handle(SegmentRemoved payload) => await Rid(payload.Id, payload.PortalId);

    public async Task Handle(SegmentsReordered payload) => await Refresh();

    public async Task Handle(SegmentMoved payload)
    {
        if (payload.OldPortalId.Equals(_portalId) && payload.OldParentId is null ||
            payload.NewPortalId.Equals(_portalId) && payload.NewParentId is null)
            await Refresh();
    }
    ...
    public override async Task Refresh(Boolean resetAlerts = true)
    {
        ...
        var response = await WithWaiting("Fetching...", () => _segmentService.FetchForPortal(request), resetAlerts);

...
public static class UserSelectByUsername
{
    public static async Task<User?> Execute(String connection, Guid? portalId, String username)
    {
        await using var command = new SqlCommand();
        command.CommandText = "FrameworkCore.UserSelectByUsername";

        command.AddParameter("@PortalId", portalId);
        command.AddParameter("@Username", 75, username);

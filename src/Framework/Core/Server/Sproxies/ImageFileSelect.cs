...
public static class ImageFileSelect
{
    public static async Task<ImageFile?> Execute(String connection, ImageFile imageFile)
    {
        await using var command = new SqlCommand();
        command.CommandText = "FrameworkCore.ImageFileSelect";

        command.AddParameter("@Id", imageFile.Id);

        return await command.ReadSingle(connection, ReadImageFile);
    }

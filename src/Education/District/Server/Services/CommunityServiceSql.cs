    ...
    : ICommunityService
{
    ...
    public async Task<Response<IList<Community>>> FetchAll(Request request)
    {
        ...
        {
            var communities = await CommunitySelectAll.Execute(Connection, request.SessionId);
    ...
    public async Task<Response> Remove(Request<Community> request)
    {
        return await wrappers.Try(request, async response =>
        {
            ...
            await sqlWrappers.WithTransaction(async (connection, transaction) =>
            {
                foreach (var communitySteward in existing.CommunityStewards)
                    await CommunityStewardDeleteByBatch.Execute(connection, transaction, request.SessionId, communitySteward);
    ...
    public async Task<Response<Community?>> FetchSchoolSelections(Request<Community> request)
    {
        ...
        {
            var community = request.Value;
            community.Schools = (await SchoolSelectSelectionsByCommunity.Execute(Connection, community.Id)).ToObservable();
            return community;
        });
    }

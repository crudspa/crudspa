    ...
    : IAssessmentAssignmentService
{
    ...
    {
        return await wrappers.Try<IList<Named>>(request, async response =>
            await ClassroomSelectNames.Execute(Connection, request.SessionId, request.Value.Id));
    }
    ...
    public async Task<Response<AssessmentAssignmentBulk>> BulkAssign(Request<AssessmentAssignmentBulk> request)
    {
        return await wrappers.Validate<AssessmentAssignmentBulk, AssessmentAssignmentBulk>(request, async response =>
        {
            ...
            switch (bulk.Scope)
            {
                case AssessmentAssignmentBulk.Scopes.EntireDistrict:
                    existingAssignments = await AssessmentAssignmentSelectForDistrict.Execute(Connection, request.SessionId, bulk.AssessmentId);
                    allStudents = await StudentSelectForDistrict.Execute(Connection, request.SessionId, bulk.AssessmentId);
                    break;
                case AssessmentAssignmentBulk.Scopes.Community:
                    existingAssignments = await AssessmentAssignmentSelectForCommunity.Execute(Connection, request.SessionId, bulk.AssessmentId, bulk.CommunityId);
                    allStudents = await StudentSelectForCommunity.Execute(Connection, request.SessionId, bulk.AssessmentId, bulk.CommunityId);
                    break;
                case AssessmentAssignmentBulk.Scopes.School:
                    existingAssignments = await AssessmentAssignmentSelectForSchool.Execute(Connection, request.SessionId, bulk.AssessmentId, bulk.SchoolId);
                    allStudents = await StudentSelectForSchool.Execute(Connection, request.SessionId, bulk.AssessmentId, bulk.SchoolId);
                    break;

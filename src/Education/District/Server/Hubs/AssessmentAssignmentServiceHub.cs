...
public partial class DistrictHub
{
    ...
    public async Task<Response<AssessmentAssignment?>> AssessmentAssignmentAdd(Request<AssessmentAssignment> request)
    {
        return await HubWrappers.RequirePermission(request, PermissionIds.Assessments, async session =>
        {
            ...
            if (response.Ok)
                await Notify(request.SessionId, PermissionIds.Assessments, new AssessmentAssignmentAdded { Id = response.Value.Id });
    ...
    public async Task<Response> AssessmentAssignmentSave(Request<AssessmentAssignment> request)
    {
        return await HubWrappers.RequirePermission(request, PermissionIds.Assessments, async session =>
        {
            ...
            if (response.Ok)
                await Notify(request.SessionId, PermissionIds.Assessments, new AssessmentAssignmentSaved { Id = request.Value.Id });
    ...
    public async Task<Response> AssessmentAssignmentRemove(Request<AssessmentAssignment> request)
    {
        return await HubWrappers.RequirePermission(request, PermissionIds.Assessments, async session =>
        {
            ...
            if (response.Ok)
                await Notify(request.SessionId, PermissionIds.Assessments, new AssessmentAssignmentRemoved { Id = request.Value.Id });
    ...
    public async Task<Response<IList<Named>>> AssessmentAssignmentFetchClassroomNames(Request<School> request)
    {
        return await HubWrappers.RequirePermission(request, PermissionIds.Assessments, async session =>
            await AssessmentAssignmentService.FetchClassroomNames(request));
    }

    public async Task<Response<IList<Named>>> AssessmentAssignmentFetchCommunityNames(Request request)
    {
        return await HubWrappers.RequirePermission(request, PermissionIds.Assessments, async session =>
            await AssessmentAssignmentService.FetchCommunityNames(request));
    }

    public async Task<Response<IList<Named>>> AssessmentAssignmentFetchSchoolNames(Request request)
    {
        return await HubWrappers.RequirePermission(request, PermissionIds.Assessments, async session =>
            await AssessmentAssignmentService.FetchSchoolNames(request));
    }

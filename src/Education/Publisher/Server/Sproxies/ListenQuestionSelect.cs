...
public static class ListenQuestionSelect
{
    public static async Task<ListenQuestion?> Execute(String connection, Guid? sessionId, ListenQuestion listenQuestion)
    {
        await using var command = new SqlCommand();
        command.CommandText = "EducationPublisher.ListenQuestionSelect";
        ...
        return await command.ExecuteQuery(connection, async reader =>
        {
            ...
            listenQuestion = ReadListenQuestion(reader);

            await reader.NextResultAsync();

            while (await reader.ReadAsync())
                listenQuestion.ListenChoices.Add(ReadListenChoice(reader));

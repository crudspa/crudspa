...
public static class VocabPartSelect
{
    public static async Task<VocabPart?> Execute(String connection, Guid? sessionId, VocabPart vocabPart)
    {
        await using var command = new SqlCommand();
        command.CommandText = "EducationPublisher.VocabPartSelect";

        command.AddParameter("@SessionId", sessionId);
        command.AddParameter("@Id", vocabPart.Id);

        return await command.ReadSingle(connection, ReadVocabPart);
    }

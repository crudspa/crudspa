...
public static class ListenPartSelect
{
    public static async Task<ListenPart?> Execute(String connection, Guid? sessionId, ListenPart listenPart)
    {
        await using var command = new SqlCommand();
        command.CommandText = "EducationPublisher.ListenPartSelect";

        command.AddParameter("@SessionId", sessionId);
        command.AddParameter("@Id", listenPart.Id);

        return await command.ReadSingle(connection, ReadListenPart);
    }

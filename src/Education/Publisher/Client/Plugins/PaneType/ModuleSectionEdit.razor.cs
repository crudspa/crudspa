...
public partial class ModuleSectionEdit : IPaneDisplay, IDisposable
{
    ...
    public ModuleSectionEditModel Model { get; set; } = null!;

    protected override async Task OnInitializedAsync()
    {
        Model = new(Path, Id, IsNew, EventBus, Navigator, ScrollService, SectionService, ItemService, ContainerService, ModuleService, ModuleId, PageId);
...
public class ModuleSectionEditModel(
    String? path,
    Guid? id,
    Boolean isNew,
    IEventBus eventBus,
    ...
    : SectionEditModel(path, id, isNew, pageId, eventBus, navigator, scrollService, sectionService, itemService, containerService)
{
    protected override async Task<Response<Section?>> FetchSection(Guid? id) =>
        await moduleService.FetchSection(new(new()
        {
            ModuleId = moduleId,
    ...
    protected override async Task<Response<Section?>> AddSection(Section section) =>
        await moduleService.AddSection(new(new()
        {
            ModuleId = moduleId,
            PageId = Entity?.PageId ?? DefaultPageId,
            Section = section,
    ...
    protected override async Task<Response> SaveSection(Section section) =>
        await moduleService.SaveSection(new(new()
        {
            ModuleId = moduleId,
            PageId = Entity?.PageId ?? DefaultPageId,
            Section = section,

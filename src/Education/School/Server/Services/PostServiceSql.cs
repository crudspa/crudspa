...
using PostDelete = Crudspa.Education.School.Server.Sproxies.PostDelete;
using PostInsert = Crudspa.Education.School.Server.Sproxies.PostInsert;
using PostSearch = Crudspa.Education.School.Shared.Contracts.Data.PostSearch;
using PostSelect = Crudspa.Education.School.Server.Sproxies.PostSelect;
using PostUpdate = Crudspa.Education.School.Server.Sproxies.PostUpdate;
    ...
    : IPostService
{
    ...
    public async Task<Response<Post?>> Fetch(Request<Post> request)
    {
        return await wrappers.Try<Post?>(request, async response =>
        {
            ...
            if (post is not null)
                post.Name = $"Post by {post.ByFirstName} {post.ByLastName?.Substring(0, 1)}.";

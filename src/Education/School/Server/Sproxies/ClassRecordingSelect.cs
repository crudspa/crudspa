...
public static class ClassRecordingSelect
{
    public static async Task<ClassRecording?> Execute(String connection, Guid? sessionId, ClassRecording classRecording)
    {
        await using var command = new SqlCommand();
        command.CommandText = "EducationSchool.ClassRecordingSelect";

        command.AddParameter("@SessionId", sessionId);
        command.AddParameter("@Id", classRecording.Id);

        return await command.ReadSingle(connection, ReadClassRecording);
    }

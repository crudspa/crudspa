    ...
    : IPublisherContactService
{
    ...
    public async Task<Response<IList<PublisherContact>>> SearchForPublisher(Request<PublisherContactSearch> request)
    {
        ...
        {
            var publisherContacts = await PublisherContactSelectWhereForPublisher.Execute(Connection, request.SessionId, request.Value);
    ...
    public async Task<Response<PublisherContact?>> Add(Request<PublisherContact> request)
    {
        return await wrappers.Validate<PublisherContact?, PublisherContact>(request, async response =>
        {
            ...
            return await sqlWrappers.WithTransaction(async (connection, transaction) =>
            {
                ...
                publisherContact.User.OrganizationId = await FetchOrganizationId(request.SessionId, publisherContact.PublisherId);
                ...
                {
                    Id = id,
                    PublisherId = publisherContact.PublisherId,
                };
            });
        });
    }
    ...
    public async Task<Response<IList<Named>>> FetchRoleNames(Request<Publisher> request)
    {
        return await wrappers.Try<IList<Named>>(request, async response =>
            await PublisherSelectRoleNames.Execute(Connection, request.SessionId, request.Value.Id));
    }
    ...
    private async Task<Guid?> FetchOrganizationId(Guid? sessionId, Guid? publisherId)
    {
        var publisher = await PublisherSelect.Execute(Connection, sessionId, new() { Id = publisherId });

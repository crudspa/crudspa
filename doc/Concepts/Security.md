...
Crudspa treats security as a layered application concern. That is important for teams evaluating the framework because the public story is not "trust the UI and hope for the best." The story is "keep the rules consistent from shell to database."
...
That is a healthier public model than making every team invent its own mix of hidden routes, scattered claims checks, and UI-only restrictions.
...
Crudspa does not wait until a user clicks something to think about permissions. Navigation can already be filtered by the current session, so users are guided toward the parts of the application that actually belong to them.

That does not replace server-side checks. It complements them. A hidden pane is helpful. A denied service call is still required. Final enforcement still matters.
...
That matters because real applications eventually hit cases where the browser is wrong, stale, or bypassed. Security still has to hold.

## Auditing

Denied access is not just a UX event. It is an operational event.
...
* filter navigation where it helps, but do not stop there
* enforce permission checks at the application boundary
* keep the final row-level rule close to the data
* treat denied access as security-relevant even when it is expected
